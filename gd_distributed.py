...
from networks.models_config import parse_option
from networks.GSformer import GSformer,GDGSformer,ViTGSformer
...
def val(val_loader, model, epoch, local_rank, args):
    ...
    with torch.no_grad():
        ...
        for  _, (images, gts,depths,grays) in enumerate(val_loader, start=1):

            gts = gts.cuda(local_rank,non_blocking=True)
            images = images.cuda(local_rank,non_blocking=True)
            depths = depths.cuda(local_rank,non_blocking=True)
            grays = grays.cuda(local_rank,non_blocking=True)

            res = model(images, torch.cat((depths,grays),dim=1))
