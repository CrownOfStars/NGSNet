...
class cal_iou(object):
    def __init__(self, num, thds=255,dilation_ratio=0.02):
        self.num = num
        self.thds = thds
        self.biou = np.zeros((self.num, self.thds))
        self.iou = np.zeros((self.num, self.thds))
        self.idx = 0
        self.dilation_ratio = dilation_ratio

    def update(self, pred, gt):
        if gt.max() != 0:
            biou,iou = self.cal(pred, gt)
            self.biou[self.idx, :] = biou
            self.iou[self.idx, :] = iou
    ...
    ########################meanF##############################
        ...
        gt_boundary = mask_to_boundary(gt, self.dilation_ratio)
        dt_boundary = mask_to_boundary(pred, self.dilation_ratio)
        
        return _iou(gt,pred),_iou(gt_boundary,dt_boundary)

    def show(self):
        assert self.num == self.idx
        biou = self.biou.mean(axis=0)
        iou = self.iou.mean(axis=0)
        return iou,biou
...
if __name__ == "__main__":
    ...
    pass
    # gt = cv2.imread("/home/data1/ShiqiangShu/NAMSwinNet/dataset/RGBD_dataset/test/COME-E/GT/COME_Easy_1.png",cv2.IMREAD_GRAYSCALE)
    # pred = cv2.imread("/home/data1/ShiqiangShu/NAMSwinNet/log/2024-01-29-21:06:27AnyNet-swin-base/save/COME-E/COME_Easy_1.png",cv2.IMREAD_GRAYSCALE)

    # cv2.imwrite("./test_iou/agt_bound.png",mask_to_boundary(gt))

    # for i in range(1,255):
    #     p = np.zeros_like(pred)
    #     p[pred>=i] = 255
    #     cv2.imwrite(f"./test_iou/dt_{i}b.png",mask_to_boundary(p))
    #     x = boundary_iou(gt,p)
    #     y = _iou(gt,p)
    #     print(i,x,y)