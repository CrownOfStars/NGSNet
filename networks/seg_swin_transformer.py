...
def swin_small_patch4_window7_224(in_chans=3,img_size=224,embed_dims=96,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    model = SwinTransformer(pretrain_img_size = img_size,in_channels=in_chans, patch_size=4, window_size=7, embed_dims=embed_dims, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    return model

def swin_tiny_patch4_window7_224(img_size=224,in_chans=3,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    model = SwinTransformer(pretrain_img_size = img_size,in_channels=in_chans, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    
    return model,[96*(2**i) for i in range(4)]



if __name__ == "__main__":
    model = swin_base_patch4_window7_224().cuda()

    
    load_pretrained_weights(model,"/home/data1/ShiqiangShu/SOLIDER/swin_base.pth","teacher","",16)
    #model.load_state_dict()
    x = torch.randn(4,3,256,224).cuda()
